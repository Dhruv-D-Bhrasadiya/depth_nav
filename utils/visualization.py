...
def draw_overlay(frame, detections, distance_map, action, reasoning, region_risks=None):
    ...
    for obj in detections:
        ...
        # Update overlay explicitly outlining group and risk!
        label = f"{cls_name} | {dist:.1f}m | {group} | risk:{risk:.1f}"
        
        # Text background
        (tw, th), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.4, 1)
        cv2.rectangle(vis_frame, (x1, y1 - th - 5), (x1 + tw, y1), color, -1)
        cv2.putText(vis_frame, label, (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0,0,0), 1)
    ...
    cv2.putText(vis_frame, f"RSN: {reasoning}", (20, h - panel_h + 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    
    if region_risks:
        risk_str = f"Aggregated Regional Risks - Left:{region_risks['LEFT']:.1f} | Center:{region_risks['CENTER']:.1f} | Right:{region_risks['RIGHT']:.1f}"
        cv2.putText(vis_frame, risk_str, (20, h - panel_h + 85), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
    
    # Resize depth map and blend
